...
### 2. 简单持仓策略 (simple_run.py)
1. **智能下单**：下一个订单并等待成交
2. **定时持仓**：持有仓位指定时间（默认60分钟）
3. **自动平仓**：时间到达后自动平仓
4. **循环运行**：支持无限循环或指定循环次数
5. **风险控制**：启动前检查现有仓位，避免重复开仓
6. **适用场景**：测试交易、短期持仓、自动化交易、持续交易
...
**简单持仓策略 (simple_run.py) 参数：**
- **quantity**: 订单交易数量
- **direction**: 交易方向，buy 表示看多，sell 表示看空
- **hold-minutes**: 持仓时间（分钟），默认60分钟
- **loops**: 循环次数（-1表示无限循环），默认-1
- **boost**: 使用市价单平仓（更快执行）
...
### 简单持仓策略 (simple_run.py)：

**基本用法（默认60分钟持仓，无限循环）：**
```bash
python simple_run.py --exchange edgex --ticker ETH --quantity 0.1 --direction buy
```

**自定义持仓时间（30分钟）：**
```bash
python simple_run.py --exchange edgex --ticker ETH --quantity 0.1 --direction buy --hold-minutes 30
```

**指定循环次数（运行5次后停止）：**
```bash
python simple_run.py --exchange edgex --ticker ETH --quantity 0.1 --direction buy --loops 5
```

**使用市价单平仓（更快执行）：**
```bash
python simple_run.py --exchange aster --ticker ETH --quantity 0.1 --direction buy --boost
```

**GRVT 交易所（支持市价单）：**
```bash
python simple_run.py --exchange grvt --ticker ETH --quantity 0.1 --direction buy --boost
```
